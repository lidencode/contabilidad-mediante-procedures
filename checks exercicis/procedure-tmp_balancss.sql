CREATE DEFINER=`root`@`localhost` PROCEDURE `tmp_balancss`(IN `_centre_id` INT, IN `_centre_exercici_id` INT, IN `_tipus_compte` VARCHAR(1), IN `_any_mes` VARCHAR(6))
	LANGUAGE SQL
	NOT DETERMINISTIC
	CONTAINS SQL
...
BEGIN
	...
	DECLARE _compte, _mes_inici, _exercici_any INT;
	...
	/* declara el cursor para comptes */
	...
	DELETE FROM tmp_balancss WHERE tmp_balancss.centre_id = _centre_id AND tmp_balancss.centre_exercici_id = _centre_exercici_id;
